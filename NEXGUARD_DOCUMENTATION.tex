...
We combine \textbf{three ML models} (CodeBERT classifier, autoencoder anomaly detector, cosine similarity) with \textbf{three GPT-4o agents} (code generation, review, documentation) to create a defense-in-depth system where:
\begin{enumerate}
...
\begin{itemize}
    \item \textbf{Node.js Backend} (TypeScript + Express) with 5 route handlers, 5 AI agents, 3 services
...
\begin{itemize}
    ...
    \item \textbf{Queue:} BullMQ 4.0 + Redis 7 (job scheduling)
...
\midrule
\texttt{/intake} & POST & Submit development ticket \\
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Generates PR descriptions from audit trail
        \item Creates incident postmortems with timeline
        \item Produces release changelogs grouped by category
    \end{itemize}
\end{enumerate}
...
\textbf{Recommended upgrade:} \texttt{microsoft/graphcodebert-base} (data-flow aware) or \texttt{microsoft/codereviewer} (trained on diffs natively)
...
\subsubsection{1. ML Model Selection}
\textbf{Challenge:} Finding a model that understands code diffs natively.

\textbf{Attempted:}
\begin{itemize}
    \item BERT base --- Poor performance (trained on natural language)
    \item GPT-3.5 classification --- Too slow and expensive for real-time
    \item CodeBERT --- Good balance of accuracy and speed
\end{itemize}

\textbf{Learning:} Domain-specific pre-training (CodeBERT on code) crucial for accuracy.

\textbf{Future:} \texttt{microsoft/codereviewer} (trained on diffs) or \texttt{graphcodebert-base} (data-flow aware) would be better.

\subsubsection{2. Synthetic Training Data Quality}
\textbf{Challenge:} GPT-4o sometimes generated unrealistic diffs.

\textbf{Issue:} Early training data had syntax errors, invalid patches.

\textbf{Solution:} Added validation pipeline:
\begin{enumerate}
    \item Syntax check generated diffs
    \item Verify patch applies cleanly
    \item Manual review of 50 samples
    \item Regenerate invalid examples
\end{enumerate}

\textbf{Learning:} Synthetic data requires validation --- LLMs hallucinate even on code.

\subsubsection{3. Autoencoder Threshold Tuning}
\textbf{Challenge:} Balancing false positives vs false negatives.

\textbf{Approach:}
\begin{itemize}
    \item Computed reconstruction errors on validation set
    \item Chose 95th percentile of GREEN errors as threshold
    \item Prefer false positives (RED $\rightarrow$ human review) over false negatives (malicious code auto-executed)
\end{itemize}

\textbf{Learning:} In security-critical systems, err on side of caution.

\subsubsection{4. GPT-4o Latency}
\textbf{Challenge:} Codex generation takes 3.2s, Reviewer adds 2.1s.

\textbf{Impact:} 5.3s just for AI agents (unacceptable for real-time).

\textbf{Attempted Solutions:}
\begin{itemize}
    \item Parallel calls --- Not possible (Reviewer depends on Codex)
    \item Smaller model --- GPT-3.5 lower quality
    \item Caching --- Works for repeated tickets (80\% hit rate)
\end{itemize}

\textbf{Learning:} GPT-4o great for quality, not speed. Consider fine-tuned smaller models (Llama 3, Mistral) for production.

\subsubsection{5. Slack Interactive Component Timeouts}
\textbf{Challenge:} Slack requires response $<$ 3 seconds, but approval transaction takes 1.5s.
...
router.post("/events", async (req, res) => {
  ...
  // Process action asynchronously (no await)
  handleApproval(proposalId, user).catch(console.error);
});
\end{lstlisting}

\textbf{Learning:} Webhook handlers must respond immediately, process async.

\subsection{Design Decisions}

\subsubsection{Why Prisma Over Raw SQL?}
\begin{itemize}
    \item \textbf{Type Safety:} Catch errors at compile time
    \item \textbf{Migrations:} Auto-generated, version controlled
    \item \textbf{Transactions:} Clean API for atomic operations
    \item \textbf{Trade-off:} Slightly slower than raw SQL (acceptable)
\end{itemize}

\subsubsection{Why Separate ML Sidecar?}
\begin{itemize}
    \item \textbf{Language Isolation:} Python for ML, Node.js for API
    \item \textbf{Scaling:} ML sidecar can scale independently
    \item \textbf{Fail-Safe:} If sidecar crashes, backend continues with rules
\end{itemize}

\subsubsection{Why Append-Only Audit Log?}
\begin{itemize}
    \item \textbf{Compliance:} Immutable record for auditors
    \item \textbf{Debugging:} Full history of every decision
    \item \textbf{ML Training:} Audit logs become future training data
\end{itemize}

\subsection{What We Would Change}

\subsubsection{1. Model Selection}
Replace CodeBERT with \texttt{microsoft/codereviewer}:
\begin{itemize}
    \item Trained on 1.3M code reviews
    \item Natively understands unified diff format
    \item Better at intent detection
\end{itemize}

\subsubsection{2. Real Training Data}
Collect real proposals from production use:
\begin{itemize}
    \item Ground truth from human approvals
    \item Org-specific patterns
    \item Better than synthetic data
\end{itemize}

\subsubsection{3. Caching Layer}
Add Redis cache for:
\begin{itemize}
    \item Embeddings (avoid re-computing)
    \item ML predictions (for identical diffs)
    \item GPT-4o responses (for repeated queries)
\end{itemize}

\subsubsection{4. Multi-Repo Support}
Current limitation: Single repo per deployment
\begin{itemize}
    \item Add \texttt{repo\_id} to models
    \item RAG context scoped per repo
    \item Per-repo policy configuration
\end{itemize}
...
\subsubsection{1. Approval Pattern Learning}
...
\subsubsection{2. Multi-Stakeholder Approval}
...
\subsubsection{3. Test Coverage Integration}
...
\subsubsection{4. Fine-Grained File Permissions}
...
\subsubsection{1. Active Learning Pipeline}
\begin{itemize}
...
\subsubsection{2. Explainability Layer}
...
\subsubsection{3. Incident Correlation}
...
\subsubsection{4. Cross-Repo Analysis}
...
\subsubsection{1. Self-Improving System}
\begin{itemize}
...
\subsubsection{2. Proactive Risk Alerts}
\begin{itemize}
...
\subsubsection{3. Federated Learning}
For SaaS deployment:
\begin{itemize}
...
\subsubsection{4. Code Generation Quality Metrics}
...
We thank the hackathon organizers for the opportunity to build this project, OpenAI for providing GPT-4o access, and the open-source community for the tools that made this possible: Prisma, FastAPI, PyTorch, HuggingFace Transformers, and many others.
...
\begin{enumerate}[leftmargin=*]
    \item Feng et al. (2020). \textit{CodeBERT: A Pre-Trained Model for Programming and Natural Languages}. EMNLP 2020.
    
    \item Guo et al. (2021). \textit{GraphCodeBERT: Pre-training Code Representations with Data Flow}. ICLR 2021.
    
    \item Li et al. (2022). \textit{CodeReviewer: Pre-Training for Automating Code Review Activities}. ArXiv 2022.
    
    \item Brown et al. (2020). \textit{Language Models are Few-Shot Learners} (GPT-3). NeurIPS 2020.
    
    \item Vaswani et al. (2017). \textit{Attention is All You Need}. NeurIPS 2017.
    
    \item GitGuardian. (2023). \textit{State of Secrets Sprawl Report}. \url{https://gitguardian.com}
    
    \item GitHub. (2023). \textit{State of the Octoverse}. \url{https://github.com/octoverse}
    
    \item OpenAI. (2024). \textit{GPT-4o System Card}. \url{https://openai.com/research/gpt-4o}
\end{enumerate}
...
% ============================================================================
\section{Appendix D: Configuration}
% ============================================================================
