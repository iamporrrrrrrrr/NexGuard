...
async function processTicket(ticket: TicketInput): Promise<{
  ...
  try {
    ...
    const matchedSignals = complexitySignals.filter((s) =>
      descLower.includes(s),
    );
    const numberMatch = description.match(
      /(\d+)\s+(file|route|module|endpoint|service)/i,
    );
    const estimatedFiles = numberMatch
      ? Math.min(parseInt(numberMatch[1]), 15)
      : Math.max(matchedSignals.length, 1);
    ...
    const isTestRelated =
      ...
      descLower.includes("coverage");
    const confidence =
      estimatedFiles > 5 ? 0.5 : estimatedFiles > 2 ? 0.65 : 0.75;
  ...
  if (riskScore.tier !== "RED") {
    ...
    const redKeywordGroups: { match: (s: string) => boolean; label: string }[] =
      [
        {
          match: (s) =>
            /remove.{0,20}(auth|guard|jwt|throttl|rate.?limit)/.test(s),
          label: "removes security mechanism",
        },
        {
          match: (s) =>
            /(plaintext|plain.text).{0,15}password|store.{0,20}password.{0,20}(plain|raw|direct)/.test(
              s,
            ),
          label: "plaintext password storage",
        },
        {
          match: (s) =>
            /hardcod.{0,15}(key|secret|credential|api|password)/.test(s),
        ...
        {
          match: (s) =>
            /child.?process|exec.{0,10}(script|command|user|arbitrary)/.test(s),
          label: "shell command execution",
        },
        {
          match: (s) => /disable.{0,15}(tls|ssl|cert)|node.tls.reject/.test(s),
          label: "TLS verification disabled",
        },
        {
          match: (s) =>
            /(bypass|disable|remove).{0,15}cors|origin.{0,10}wildcard|allow.{0,10}all.{0,10}origin/.test(
              s,
            ),
          label: "CORS bypass",
        },
        {
          match: (s) =>
            /console\.log.{0,30}(password|credential|secret|token)/.test(s),
          label: "credential logging",
        },
        {
          match: (s) =>
            /remove.{0,20}(validation|validatepipe|validation.?pipe)/.test(s),
          label: "removes input validation",
        },
        {
          match: (s) =>
            /delete.{0,10}all.{0,10}(user|session|data)|nuke.{0,10}(user|data)/.test(
              s,
            ),
          label: "destructive data deletion",
        },
        {
          match: (s) =>
            /(expose|return|log).{0,20}(stack.?trace|exception\.stack)/.test(s),
          label: "stack trace exposure",
        },
        {
          match: (s) =>
            /string.{0,10}concat.{0,20}(sql|query)|sql.{0,20}string.{0,10}concat/.test(
              s,
            ),
        ...
        {
          match: (s) =>
            /expose.{0,20}(database.?url|connection.?string|env.?var.{0,10}(in.response|api|endpoint))/.test(
              s,
            ),
          label: "exposes secrets via API",
        },
        {
          match: (s) =>
            /(remove|skip|without).{0,20}test(s|\b)|no.{0,10}test.step/.test(s),
          label: "skips test execution",
        },
      ];
    ...
    for (const { match, label } of redKeywordGroups) {
      if (match(descLower)) {
        riskScore = {
          ...
          reasons: [
            `Description indicates high-risk intent: ${label}`,
            ...riskScore.reasons,
          ],
  ...
  embedAndStore(saved.id, `${proposal.summary} ${proposal.diff}`, {
    tier: riskScore.tier,
    repo: ticket.repo,
  }).catch((err) =>
    console.warn("[intake] embedAndStore failed:", err.message),
  );
  ...
  if (riskScore.tier === "GREEN") {
    try {
      ...
      await writeAuditLog("AUTO_EXECUTED", "system", saved.id, {
        tier: "GREEN",
      });
    ...
    try {
      ...
      console.warn(
        "[intake] sendApprovalCard failed:",
        (slackErr as Error).message,
      );
    }
    await writeAuditLog("APPROVAL_SENT", "system", saved.id, {
      tier: riskScore.tier,
    });
    await prisma.proposal.update({
      where: { id: saved.id },
  ...
  try {
    ...
    if (!title || !description || !repo || !reporter) {
      res
        .status(400)
        .json({
          error: "Missing required fields: title, description, repo, reporter",
        });
      return;
    }
  ...
  try {
    ...
    if (!issue?.fields) {
      res
        .status(400)
        .json({
          error: "Invalid Jira webhook payload — expected issue.fields",
        });
      return;
    }
    ...
    if (!ticket.repo) {
      res.status(400).json({
        error:
          "Could not determine repo. Add a label like 'repo:org/name' to the Jira issue, or set a 'Repository' custom field.",
      });
      return;
    }

    console.log(
      `[Jira] ${payload.webhookEvent || "webhook"}: ${issue.key} → "${ticket.title}" → ${ticket.repo}`,
    );
