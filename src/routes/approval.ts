...
function renderHtmlPage(title: string, emoji: string, message: string, detail: string = ""): string {
...
router.get("/approve/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (!existingProposal) {
      if (wantsBrowserHtml(req)) return res.status(404).send(renderHtmlPage("Not Found", "❌", "Proposal not found."));
      return res.status(404).json({ error: "Proposal not found" });
    }

    if (existingProposal.status !== "AWAITING_APPROVAL") {
      if (wantsBrowserHtml(req)) return res.status(400).send(renderHtmlPage("Already Handled", "⚠️", `This proposal was already ${existingProposal.status.toLowerCase().replace(/_/g, " ")}.`));
    ...
    if (wantsBrowserHtml(req)) {
      return res.status(200).send(renderHtmlPage(
        ...
        prUrl ? `PR created: <a href="${prUrl}" style="color:#38bdf8">${prUrl}</a>` : "Execution pending."
      ));
    }
    res.status(200).json({
      success: true,
...
router.get("/diff/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (wantsBrowserHtml(req)) {
      const tierColors: Record<string, string> = { GREEN: "#22c55e", YELLOW: "#eab308", RED: "#ef4444" };
      const tierColor = tierColors[proposal.tier] || "#64748b";
      const escapedDiff = proposal.diff.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
...
<div class="diff-container"><pre>${escapedDiff.split("\n").map((line: string) => {
        if (line.startsWith("+")) return `<span class="line-add">${line}</span>`;
        if (line.startsWith("-")) return `<span class="line-del">${line}</span>`;
        if (line.startsWith("@@")) return `<span class="line-hunk">${line}</span>`;
        return line;
      }).join("\n")}</pre></div>
</body></html>`;
